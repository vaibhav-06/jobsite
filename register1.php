...
if (isset($_POST['submit'])) {
	...
	$check_query = mysqli_query($conn, "SELECT * FROM jobcreator where email ='$email'");
	...
	if ($password == $cpassword) {
		...
		if(!$check){
			...
			<script>
				...
				window.location.replace('otp1.php');
...
<body style="background-color:lavender;">
	<div class="container">
		<form action="" method="POST" class="login-email" name="registrationform" onsubmit="return validateform()" >
			...
			<p style="color:blue;">Have an account? <a href="login1.php">Login Here</a>.</p>
		</form>
	</div>
</body>
</html>